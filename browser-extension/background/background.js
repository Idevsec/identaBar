        ...
        async function tryQueryRegistry(queryHost) {
            try {
                ...
                const response = await fetch(
                    `https://creduent.idevsec.com/agents?query=${encodeURIComponent(queryHost)}`,
                    {
                        signal: controller.signal,
                    }
                );
                ...
                if (response.ok) {
                    ...
                    if (Array.isArray(matchingAgents) && matchingAgents.length > 0) {
                        const attestationPromises = matchingAgents.map((agent) =>
                            fetchRegistryAttestation(agent.agent_id, domain)
                        );
...
async function fetchRegistryAttestation(agentId, domain) {
    try {
        ...
        const validation = await CreduentVerification.fetchAndVerifyRegistryAttestation(
            cleanAgentId,
