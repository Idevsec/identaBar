...
verifyRunBtn.addEventListener("click", async () => {
    ...
    if (signatureB64.length !== 88) {
        printConsoleLog(
            `Warning: Ed25519 Base64 signature is expected to be exactly 88 characters. Yours is ${signatureB64.length} characters.`,
            "error"
        );
    }
    ...
    try {
        ...
        if (verified) {
            ...
            printConsoleLog(
                "The message was signed by the private key corresponding to this agent public key.",
                "success"
            );
        } else {
            printConsoleLog("✕ SIGNATURE VALIDATION: FAILED!", "error");
            printConsoleLog(
                "Warning: Signature does not match the public key or message content was altered.",
                "error"
            );
        }
    } catch (err) {
